...
from System import Array as arr
...
class __namedtuple(IEnumerable):
    ...
    def __find_by_name(self, name):
        try:
            index = self.myattributes.index(name)
        except ValueError as ex:
            raise ValueError(
                str.format(
                'namedtuple does not have an attribute for \'{}\'. Available: {}',
                name, str(self.myattributes))
                )
...
def __make_function__(info, inputnames, outputnames, keep_tree):
    def component_function(*args, **kwargs):
        ...
        if args:
            ...
            import System
            args = System.Array[object](len(inputnames))
            for kwarg in kwargs:
                if kwarg in inputnames:
                    index = inputnames[kwarg]
                    args[index] = kwargs[kwarg]
                else:
                    raise ValueError(
                        str.Format("{0} argument is not a valid parameter of the function. Valid parameters are: {1}", kwarg, inputnames))
            result, warnings = info.Evaluate(args, keep_tree)
        else:
            info.Evaluate(None, keep_tree)
        if result:
            if not keep_tree:
...
def __build_module():
    ...
    for info_names in ghc.NodeInCodeFunctions.GetDynamicMemberNames():
        ...
        try:
            ...
            inputnames = [el.lower().replace(" ", "_") for el in info.InputNames]
            inputnames_dict = dict((el,i) for i, el in enumerate(inputnames))
            outputnames = [el.lower().replace(" ", "_") for el in info.OutputNames]
